!function () {
   ...
   * 计算自动滚动屏幕居中
   */
  let rem = parseFloat(document.querySelector('html').style.fontSize);
  let scrollX = 1.86 / 2 * rem;
  let scrollIng = false;
  ...
  document.addEventListener('touchmove', function (e) {
    if (e.target.classList.contains('scale')) {
      e.target.parentNode.style.width =
        parseFloat(e.target.dataset['startW']) +
        e.touches[0].clientX -
        parseFloat(e.target.dataset['startX']) + 'px';
    }
  })
